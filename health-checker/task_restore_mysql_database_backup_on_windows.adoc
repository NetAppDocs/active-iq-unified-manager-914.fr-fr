...
+
Il est recommandé de copier le fichier de sauvegarde dans le répertoire par défaut `\ProgramData\NetApp\OnCommandAppData\ocum\backup` . Les fichiers du référentiel de base de données doivent être copiés dans le `\database_dumps_repo` sous-répertoire situé sous le `\backup` répertoire.
