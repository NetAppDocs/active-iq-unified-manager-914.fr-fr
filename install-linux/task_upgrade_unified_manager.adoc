...
* Pour éviter les pertes de données, vous devez avoir créé une sauvegarde de la base de données Unified Manager en cas de problème lors de la mise à niveau. NetApp vous recommande de déplacer le fichier de sauvegarde du `/opt/netapp/data` répertoire vers un emplacement externe.
...
+
Les paquets sont téléchargés dans le répertoire spécifié comme `--downloaddir=`.
