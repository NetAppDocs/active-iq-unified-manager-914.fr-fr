...
* 100 Go d'espace disque disponible alloué au `/opt/netapp/data` répertoire, qui est monté sur un lecteur LVM ou sur un disque local distinct connecté au système cible


[NOTE]
====
Pour les répertoires et `/var/log` montés séparément `/opt`, assurez-vous que `/opt` dispose de 15 Go et `/var/log` dispose de 16 Go d'espace libre. Le `/tmp` répertoire doit disposer d'au moins 10 Go d'espace libre.
