...
La question des collections de statistiques de performance qui ne se terminent pas à temps est indiquée par les messages de la bannière `Unable to consistently collect from cluster <cluster_name> or Data collection is taking too long on cluster <cluster_name>.`
