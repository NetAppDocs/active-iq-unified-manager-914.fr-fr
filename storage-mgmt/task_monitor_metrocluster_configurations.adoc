...
Pour plus d'informations sur la détermination de l'intégrité du cluster, reportez-vous à link:../health-checker/task_check_health_of_clusters_in_metrocluster_configuration.html#determining-cluster-health-in-metrocluster-over-fc-configuration["Détermination de l'état du cluster dans la configuration MetroCluster sur FC"]la section .



== Surveiller les problèmes de connectivité dans la configuration MetroCluster over IP
...
. Développez la ligne en cliquant sur l'icône caret `v`. L'icône caret s'affiche uniquement pour un cluster protégé par la configuration MetroCluster over IP.
+
...
NOTE: Les États suivants sont signalés : critique (image:sev_critical_um60.png[""]), erreur ( ) ou Normal (image:sev_normal_um60.png[""] )image:sev_error_um60.png[""]. Vous pouvez également afficher l'état de réplication des données de l'agrégat des données primaires et en miroir dans une même topologie.
