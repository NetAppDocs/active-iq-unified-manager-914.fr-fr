...
+
Pour afficher les noms des événements EMS auxquels vous pouvez vous abonner, vous pouvez utiliser la commande (avant ONTAP 9) ou la commande (ONTAP 9 ou version ultérieure) dans le shell de `event catalog show` cluster ONTAP `event route show`.
