...
. Lorsque vous y êtes invité, tapez `*y*` pour continuer.
...
. Lorsque vous y êtes invité, tapez `*y*` pour vérifier que les informations que vous avez saisies sont correctes.
...
ID de groupe de statistiques
 a| 
`*0*` - INDICATEUR_PERFORMANCES (par défaut)

`*1*` - UTILISATION_DES_RESSOURCES
...
. Lorsque vous y êtes invité, tapez `*y*` pour vérifier que les informations que vous avez saisies sont correctes.
. Appuyez sur n'importe quelle touche pour revenir au menu connexion au serveur externe.
. Tapez `*x*` pour quitter la console de maintenance.
